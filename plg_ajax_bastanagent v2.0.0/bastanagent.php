...
class PlgAjaxBastanagent extends CMSPlugin {
    ...
    public function onAjaxBastanagent() {
        ...
        // --- Start Security Firewalls Check ---

        // 1. Check RSFirewall logs
        try {
            $query = $db->getQuery(true)
            ...
            if (is_array($logs)) {
                ...
                $response['firewall_type'] = 'RSFirewall';
                $response['security_logs'] = $logs;
            }
        } catch (Exception $e) {}

        // 2. Check Admin Tools (if RSFirewall is not present)
        if (!$response['has_firewall']) {
            try {
                // Extract from WAF table and map column names to system standards
                $query = $db->getQuery(true)
                    ->select($db->quoteName('LogDate', 'date') . ', ' . $db->quoteName('IP', 'ip') . ', ' . $db->quoteName('Reason', 'page'))
                ...
                if (is_array($logs)) {
                    $response['has_firewall'] = true;
                    $response['firewall_type'] = 'Admin Tools';
                    $response['security_logs'] = $logs;
                }
            } catch (Exception $e) {
                // If it's the Core version and lacks a log table, just check if it's installed
                try {
                    $query = $db->getQuery(true)->select('extension_id')->from('#__extensions')->where('element = "com_admintools"');
                    $db->setQuery($query);
                    if ($db->loadResult()) {
                        $response['has_firewall'] = true;
                        $response['firewall_type'] = 'Admin Tools';
        ...
        // 3. Check HTProtect (if previous firewalls are not present)
        if (!$response['has_firewall']) {
            try {
                // Map column names for compatibility with the monitor
                $query = $db->getQuery(true)
                    ->select($db->quoteName('created_at', 'date') . ', ' . $db->quoteName('ip', 'ip') . ', ' . $db->quoteName('reason', 'page'))
                    ->from($db->quoteName('#__htprotect_logs'))
                    ->order($db->quoteName('id') . ' DESC');
                $db->setQuery($query, 0, 5);
                $logs = $db->loadAssocList();
                if (is_array($logs)) {
                    $response['has_firewall'] = true;
                    $response['firewall_type'] = 'HTProtect';
                    $response['security_logs'] = $logs;
                }
            } catch (Exception $e) {
                // If log table structure differs, at least verify it is active
                try {
                    $query = $db->getQuery(true)->select('extension_id')->from('#__extensions')->where('element = "com_htprotect"');
                    $db->setQuery($query);
                    if ($db->loadResult()) {
                        $response['has_firewall'] = true;
                        $response['firewall_type'] = 'HTProtect';
        ...
        // --- End Security Firewalls Check ---


        // Check for pending updates (with language package filter)
        try {
            $query = $db->getQuery(true)
                ->select($db->quoteName(['element', 'version', 'type'])) // Added type column
            ...
            if ($updates) {
                foreach ($updates as $upd) {
                    if (!empty($upd['element'])) {
                        ...
                        // Filter language packages
                        if (isset($upd['type']) && $upd['type'] === 'language') continue;
                        if (preg_match('/^pkg_[a-z]{2}-[a-zA-Z]{2}$/i', $upd['element'])) continue;
