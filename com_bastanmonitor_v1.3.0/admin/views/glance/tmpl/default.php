                    ...
                    <div class="card h-100 shadow-sm border-0 position-relative bm-card">
                        ...
                        <div class="card-body text-center p-3 pt-4">
                            ...
                            <div class="mb-3 d-flex justify-content-center gap-1">
                                <span class="badge bg-secondary"><?php echo Text::_('COM_BASTANMONITOR_GLANCE_CMS_LABEL'); ?>: <?php echo $data['version'] ?? ($site->cms_version ?: Text::_('COM_BASTANMONITOR_UNKNOWN')); ?></span>
                                <span class="badge bg-dark"><?php echo Text::_('COM_BASTANMONITOR_GLANCE_PHP_LABEL'); ?>: <?php echo $data['php_version'] ?? ($site->php_version ?: Text::_('COM_BASTANMONITOR_UNKNOWN')); ?></span>
                            </div>
                        ...
                        <div class="modal-content">
                            ...
                            <div class="modal-body text-start">
                                <div class="d-flex justify-content-between align-items-center mb-2 mt-3">
                                    <h6 class="fw-bold text-primary mb-0">📦 <?php echo Text::_('COM_BASTANMONITOR_MODAL_PENDING_UPDATES'); ?>:</h6>
                                    
                                    <?php if (!empty($data['pending_updates'])) : ?>
                                        <?php 
                                            // Auto-detect update path based on synced CMS data instead of URL
                                            $cmsType = isset($data['cms']) ? strtolower($data['cms']) : '';
                                            $isWp = (strpos($cmsType, 'word') !== false || strpos($cmsType, 'wp') !== false);
                                            
                                            $updatePath = $isWp ? '/wp-admin/update-core.php' : '/administrator/index.php?option=com_installer&view=update';
                                            $targetUrl = rtrim($site->url, '/') . $updatePath;
                                        ?>
                                        <a href="<?php echo $targetUrl; ?>" target="_blank" class="btn btn-sm btn-primary shadow-sm">
                                            🚀 <?php echo Text::_('COM_BASTANMONITOR_MODAL_LOGIN_UPDATE_BTN'); ?>
                                        </a>
                                    <?php endif; ?>
                                </div>
                                ...
                                <?php if (!empty($data['pending_updates'])) : ?>
                                    <ul class="list-group mb-3">
                                        <?php foreach ($data['pending_updates'] as $key => $update) : ?>
                                            <?php 
                                                // Support for both old and new data structures
                                                $extName = is_array($update) ? $update['element'] : $key;
                                                $extVer  = is_array($update) ? $update['version'] : $update;
                                            ?>
                                            <li class="list-group-item d-flex justify-content-between align-items-center">
                                                <span><?php echo htmlspecialchars($extName); ?></span>
                                                <span class="badge bg-info text-dark"><?php echo Text::_('COM_BASTANMONITOR_MODAL_NEW_VERSION'); ?>: <?php echo htmlspecialchars($extVer); ?></span>
                                            </li>
                                        <?php endforeach; ?>
                                    </ul>
                                <?php else : ?>
                                    ...
                                    <ul class="list-group mb-3">
                                        <?php foreach ($data['security_logs'] as $log) : ?>
                                            <li class="list-group-item small">
                                                <span><?php echo Text::_('COM_BASTANMONITOR_MODAL_IP_LABEL'); ?>: <strong><?php echo $log['ip'] ?? Text::_('COM_BASTANMONITOR_UNKNOWN'); ?></strong></span> | 
